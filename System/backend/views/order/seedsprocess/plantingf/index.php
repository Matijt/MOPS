...
$this->params['breadcrumbs'][] = "Sowing orders Female";
?>
<div class="order-index">

    <h1>Sowing orders Female</h1>

<?php
// Seeds arrive
$seedsArrive = Order::find()->joinWith(['hybridIdHybr'])->where('order.state = "Male seeds planted"')->all();

$date = date('Y-m-d');
$proximaSeamana = date('Y-m-d', strtotime("$date + 7 day"));
$semanaPasada = date('Y-m-d', strtotime("$date - 7 day"));
foreach($seedsArrive AS $item){
    $fechaEvaluada = date('Y-m-d',  strtotime($item->sowingDateF));
    ...
    switch ($fechaEvaluada) {
        case $primero:
            ...
            continue;
        case $segundo:
            $item->action = 'Remember you should plant '.$item->hybridIdHybr->motherIdMother->variety.' in the compartment '.$item->compartmentIdCompartment->compNum.' In the next days';
            continue;
        case $tercero:
            $item->action = 'Plant '.$item->sowingF.' '.$item->hybridIdHybr->motherIdMother->variety.' in the compartment '.$item->compartmentIdCompartment->compNum.' TODAY';
            continue;
        case $cuarto:
            $item->action = 'YOU SHOULD HAVE PLANTED '.$item->sowingF.' '.$item->hybridIdHybr->motherIdMother->variety.' in the compartment '.$item->compartmentIdCompartment->compNum." already";
            continue;
        case $quinto:
            $item->action = 'YOU REQUIRED TO PLANT '.$item->sowingF.' '.$item->hybridIdHybr->motherIdMother->variety.' IN THE COMPARTMENT '.$item->compartmentIdCompartment->compNum.' ON THE DAY '.$fechaEvaluada." IT IS LATE";
            continue;
    }
            ...
            ['class' => 'yii\grid\ActionColumn',
                ...
                'buttons' => [
                    ...
                    'update' => function ($url, $model) {
                        return Html::button('<span class="glyphicon glyphicon-pencil"></span>', ['value' => Url::to('index.php?r=order%2Fupdate&id='.$model->idorder.'&name=plantingf'), 'class' => 'modalButtonEdit'], [
                            'title' => Yii::t('app', 'Update'),
                        ]);
                    },

                    'delete' => function ($url, $model) {
                        return Html::a('<span class="glyphicon glyphicon-trash"></span>', 'index.php?r=order/delete&id='.$model->idorder.'&name=plantingf', [
                            'data-method' => 'POST',
                            'title' => Yii::t('app', 'Update'),
