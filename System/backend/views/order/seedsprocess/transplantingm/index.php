...
$this->params['breadcrumbs'][] = "Transplanting orders Male";
?>
<div class="order-index">

    <h1>Transplanting orders Male</h1>
    <?php
...
<?php
// Seeds arrive
$seedsArrive = Order::find()->joinWith(['hybridIdHybr'])->where('order.state = "female seeds planted"')->all();

$date = date('Y-m-d');
$proximaSeamana = date('Y-m-d', strtotime("$date + 7 day"));
$semanaPasada = date('Y-m-d', strtotime("$date - 7 day"));
foreach($seedsArrive AS $item){
    $fechaEvaluada = date('Y-m-d',  strtotime($item->transplantingM));
    ...
    switch ($fechaEvaluada) {
        case $primero:
            ...
            continue;
        case $segundo:
            $item->action = 'Remember you should transplant '.$item->hybridIdHybr->fatherIdFather->variety.' to the compartment '.$item->compartmentIdCompartment->compNum.' In the next days';
            continue;
        case $tercero:
            $item->action = 'Transplant '.$item->sowingM.' '.$item->hybridIdHybr->fatherIdFather->variety.' to the compartment '.$item->compartmentIdCompartment->compNum.' TODAY';
            continue;
        case $cuarto:
            $item->action = 'YOU SHOULD HAVE TRANSPLANTED '.$item->sowingM.' '.$item->hybridIdHybr->fatherIdFather->variety.' to the compartment '.$item->compartmentIdCompartment->compNum." already";
            continue;
        case $quinto:
            $item->action = 'YOU REQUIRED TO TRANSPLANT '.$item->sowingM.' '.$item->hybridIdHybr->fatherIdFather->variety.' TO THE COMPARTMENT '.$item->compartmentIdCompartment->compNum.' ON THE DAY '.$fechaEvaluada." IT IS LATE";
            continue;
    }
            ...
            ['class' => 'yii\grid\ActionColumn',
                ...
                'buttons' => [
                    ...
                    'update' => function ($url, $model) {
                        return Html::button('<span class="glyphicon glyphicon-pencil"></span>', ['value' => Url::to('index.php?r=order%2Fupdate&id='.$model->idorder.'&name=transplantingm'), 'class' => 'modalButtonEdit'], [
                            'title' => Yii::t('app', 'Update'),
                        ]);
                    },

                    'delete' => function ($url, $model) {
                        return Html::a('<span class="glyphicon glyphicon-trash"></span>', 'index.php?r=order/delete&id='.$model->idorder.'&name=transplantingm', [
                            'data-method' => 'POST',
                            'title' => Yii::t('app', 'Update'),
