...
class OrderSearch extends Order
{
    ...
    {
        return [
            [['numCrop', 'numRowsOpt', 'idorder', 'numRows', 'netNumOfPlantsF', 'netNumOfPlantsM', 'contractNumber', 'sowingM', 'sowingF', 'nurseryM', 'nurseryF', 'realisedNrOfPlantsM', 'realisedNrOfPlantsF', 'extractedPlantsF', 'extractedPlantsM', 'remainingPlantsF', 'remainingPlantsM', 'pollenColectQ','plantingDistance','calculatedYield', 'gpOrder'], 'integer'],
            [['orderKg'], 'number'],
            [['ReqDeliveryDate', 'orderDate', 'ssRecDate', 'check', 'sowingDateM', 'sowingDateF', 'transplantingM', 'transplantingF', 'pollenColectF', 'pollenColectU', 'pollinationF', 'pollinationU', 'harvestF', 'harvestU', 'steamDesinfectionF', 'steamDesinfectionU', 'remarks', 'compartment_idCompartment', 'Hybrid_idHybrid','state','action', 'prueba', 'prueba2', 'canceledDate'], 'safe'],
        ];
    }
    ...
    public function search($params)
    {
        ...
        if($this->canceledDate) {
            $this->canceledDate = date('Y-m-d', strtotime($this->canceledDate));
        }

        $query->joinWith('compartmentIdCompartment');
        $query->joinWith('hybridIdHybr');
        $query->joinWith('hybridIdHybr.fatherIdFather');
        $query->joinWith('hybridIdHybr.motherIdMother');
        ...
        $query->andFilterWhere(['like', 'check', $this->check])
            ...
            ->andFilterWhere(['like', 'canceledDate', $this->canceledDate])
            ->andFilterWhere(['like', 'father.variety', $this->prueba])
            ->andFilterWhere(['like', 'mother.variety', $this->prueba2])
        ...
        IF($this->canceledDate) {
            $this->canceledDate = date('d-m-Y', strtotime($this->canceledDate));
        }
